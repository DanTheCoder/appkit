...
const backdropClickAnimate = ref("");

const sizeClass = computed({
  get() {
    return props.size ? props.size : "sm:max-w-xl";
  },
});

const handleBackdropClose = () => {
  if (props.backdropClose) {
    return emit("update:open", !props.open);
  }

  backdropClickAnimate.value = "scale-105";
  setTimeout(() => {
    backdropClickAnimate.value = "";
  }, 300);
};
</script>
            ...
            <DialogPanel :class="sizeClass" class="w-full relative bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all">
              <div class="p-4 sm:p-6">
                <div class="block absolute top-0 right-0 pt-4 pr-4">
                  <button tabindex="-1" type="button" class="bg-white dark:bg-gray-800 rounded-md text-gray-400 hover:text-gray-500 focus:outline-none" @click="$emit('update:open', !open)">
